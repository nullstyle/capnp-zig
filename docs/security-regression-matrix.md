...
| RPC message tag and payload pointer malformation | `src/rpc/level0/protocol.zig` | `zig build test-rpc-level0`, `tests/rpc/level0/rpc_protocol_test.zig` | Covered |
| RPC cap-table descriptor confusion or invalid hosted IDs | `src/rpc/level0/cap_table.zig`, `src/rpc/level3/peer.zig` | `zig build test-rpc-level0`, `zig build test-rpc-level3` | Covered |
| Promised-answer transform length and path validation | `src/rpc/level1`, `src/rpc/level3` | `tests/rpc/level1/rpc_promised_answer_transform_test.zig`, level3 peer tests | Covered |
| Duplicate inbound/outbound questions and release over-counts | `src/rpc/level3/peer.zig` | `tests/rpc/level3/rpc_release_and_failure_test.zig`, `tests/rpc/level3/rpc_peer_test.zig` | Covered |
| Peer pending-map resource exhaustion | `src/rpc/level3/peer.zig` | `zig build test-rpc-level3`, `zig build test-resource-budgets`, `zig build test-oom` | Covered |
| Return/resolve/embargo failure atomicity under OOM or callback failure | `src/rpc/level3/peer/**` | `tests/rpc/level3/rpc_peer_from_peer_zig_test.zig`, `tests/rpc/level3/rpc_release_and_failure_test.zig` | Covered |
| TCP transport frame budget, close, and callback cleanup behavior | `src/rpc/level2/connection.zig`, `src/rpc/level2/transport.zig` | `tests/rpc/level2/rpc_connection_failure_test.zig`, `tests/rpc/level2/rpc_peer_cleanup_test.zig` | Covered |
| QUIC receive/send budget and production hardening controls | `src/rpc/level2/quic_transport.zig` | `tests/rpc/level2/rpc_quic_transport_test.zig`, `zig build test-resource-budgets` | Covered |
| HostPeer queue budgets and default external exception/abort disclosure | `src/rpc/integration/host_peer.zig` | `tests/rpc/level2/rpc_host_peer_test.zig` | Covered |
