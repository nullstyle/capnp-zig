...
| L4 JoinResult direct-Accept rollback, addressed registry handling, and token lifetime | `src/rpc/peer/mod.zig`, `src/rpc/vat/join.zig` | `tests/rpc/peer/rpc_join_readiness_test.zig`, `src/rpc/vat/join.zig` addressed-network tests, `just e2e-l4-zig` real TCP gate | Covered |
