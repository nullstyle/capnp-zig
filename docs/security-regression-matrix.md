...
| L4 JoinCoordinator origination, accepted-cap ownership, per-peer Finish, cancel cleanup, and JoinResult lifetime | `src/rpc/peer/mod.zig` | `tests/rpc/peer/rpc_join_readiness_test.zig` coordinator happy path, duplicate local part rejection, post-JoinResult and post-Accept-send cancel cleanup, proxy-relay pickup through the real coordinator, and sendPart OOM rollback; `zig build test-rpc-peer`, `zig build test-oom` | Covered |
