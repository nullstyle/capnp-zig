...
| L4 JoinCoordinator origination, accepted-cap ownership, cancel cleanup, and JoinResult Finish lifetime | `src/rpc/peer/mod.zig` | `tests/rpc/peer/rpc_join_readiness_test.zig` coordinator happy path, post-JoinResult cancel cleanup, and sendPart OOM rollback; `zig build test-rpc-peer`, `zig build test-oom` | Covered |
| L4 transparent proxy Join relay teardown and cancellation lifetime | `src/rpc/peer/mod.zig`, `src/rpc/vat/join.zig` | `tests/rpc/peer/rpc_join_readiness_test.zig` proxy relay success, Finish-before-Return, source unavailable, send failure, owner/source teardown, mismatch, and OOM rollback cases; `zig build test-rpc-peer` | Covered |
