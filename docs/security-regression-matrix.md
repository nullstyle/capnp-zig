...
| Retained outbound-answer admission, explicit/transferred Finish ownership, synchronous Return, and retry cleanup | `src/rpc/peer/mod.zig`, `src/rpc/peer/retained_questions.zig` | `tests/rpc/peer/rpc_deadline_test.zig` retained lifetime/limit/events/synchronous Return/close cases; `tests/rpc/peer/rpc_peer_alloc_failure_test.zig`; `tests/rpc/peer/rpc_three_party_handoff_origination_test.zig` retained promised-answer transfer, fallback/direct pickup, manual-Finish rejection, and exact drain; `zig build test-rpc-peer`, `zig build test-resource-budgets`, `zig build test-oom` | Covered |
| Return/resolve/embargo failure atomicity under OOM, callback failure, or reentrant Finish | `src/rpc/peer/**` | `tests/rpc/peer/rpc_peer_from_peer_zig_test.zig`, `tests/rpc/peer/rpc_release_and_failure_test.zig`, `tests/rpc/peer/rpc_answer_lifecycle_test.zig` reentrant Finish during results and Bootstrap Return sends with queued promised-answer drain (in both Finish orderings), `releaseResultCaps` on a late Return after a cancelling Finish, rejection of question-id reuse over an undischarged early-Finish tombstone, and nested resolved-answer reservations at the answer map's load-factor boundary | Covered |
| L3 three-party auto-pickup Accept Finish ordering, retry, and cleanup | `src/rpc/peer/mod.zig` | `tests/rpc/peer/rpc_three_party_handoff_pickup_test.zig` callback-failure cleanup and first internal Accept Finish OOM retry after synchronous host-answer commit; `zig build test-rpc-peer`, `zig build test-oom` | Covered |
| L3 unauthenticated parked-Accept fairness, attributable-byte admission, expiry, and terminal-close cleanup | `src/rpc/vat/provisions.zig`, `src/rpc/vat/host.zig`, `src/rpc/peer/mod.zig`, `src/rpc/integration/host_peer.zig` | `tests/rpc/peer/rpc_three_party_handoff_vatc_test.zig` deterministic per-peer/vat-wide count+byte limits, repeated-token charging, deadline boundary/ordinary-frame sweep, adoption/Finish/close/index/peer refund, reentrancy and OOM cases; `tests/rpc/integration/rpc_host_peer_test.zig` idempotent manual close; `zig build test-rpc-l3`, resource-budget/OOM/ReleaseSafe/ReleaseFast gates; `just e2e-l3-vatc` bounded C++ fairness/recovery | Covered |
