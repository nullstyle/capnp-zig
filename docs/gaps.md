...
- **Streaming flow control**: `StreamClient` with fire-and-forget calls, error sealing, drain notification, `StreamState` runtime (GAP-2 resolved)
...
**All 7 Arena scenarios are fully ready.** 3 scenarios are trivially ready (ping, echo, transfer). 3 more are fully supported with generated typed helpers (getChain, getFanout, collaborate). The stream scenario is supported via `StreamClient` with fire-and-forget calls, error sealing, and drain notification.
...
- Codegen: `StreamClient` with fire-and-forget calls, error sealing, `waitStreaming()`
- Runtime: `StreamState` tracks in-flight calls, caches first error, drain notification
...
### GAP-2: Streaming Flow Control -- RESOLVED
...
- Server side unchanged: single-threaded event loop serializes handler calls, auto-ack via `sendReturnEmptyStruct`
...
| stream | READY | None | StreamClient + error sealing + drain |

| GAP | Status | Severity |
|-----|--------|----------|
| GAP-1: Promise pipelining | RESOLVED | -- |
| GAP-2: Streaming | RESOLVED | -- |
...
**Bottom line:** All 7 Arena scenarios are fully ready. No remaining significant work items.
