...
- 2026-02-08: Expanded `src/rpc/transport_xev.zig` failure-path coverage with EOF-close signaling, explicit close-error propagation, and write-completion behavior when callback context is missing.
...
- 2026-02-08: Reduced duplicated return-send plumbing in `src/rpc/peer/return/peer_return_dispatch.zig` by centralizing clear/send/free behavior and adding dedicated tests for exception and `takeFromOtherQuestion` frame emission.
- 2026-02-08: Optimized `MessageBuilder` write paths in `src/message.zig` by pre-reserving root storage, using contiguous text writes, and pre-sizing `toBytes()` output/header emission to reduce allocation churn without changing wire semantics.
...
- 2026-02-08: Continued RPC duplication cleanup by centralizing promised-answer op-slice copying in `src/rpc/promised_answer_copy.zig` and reusing it from `src/rpc/cap_table.zig`; added alias-safety coverage in `tests/rpc_cap_table_encode_test.zig`.
