...
- 2026-02-06: Implemented baseline `provide`/`accept`/`join` runtime semantics and wired dedicated `tests/rpc/level3/rpc_peer_test.zig` into `zig build test-rpc` to validate provision success, duplicate recipient abort, finish cleanup, join return, and current `thirdPartyAnswer` abort behavior.
...
- 2026-02-06: Replaced stub codegen type mappings in `src/capnpc-zig/types.zig`/`src/capnpc-zig/generator.zig` with concrete runtime reader types, added `List(Void)` reader/builder generation, and generated pointer-field helper builders (`clear*`, `set*Capability`, `set*Text`, `set*Data`, `set*Null`) with coverage in `tests/serialization/codegen_test.zig`.
- 2026-02-06: Fixed generated struct setter default encoding to match Cap'n Proto xor semantics for bool/numeric/float/enum fields, including fallback `u16` handling for unresolved enum types; added regression assertions in `tests/serialization/codegen_defaults_test.zig` and `tests/serialization/codegen_test.zig`.
...
- 2026-02-06: Fixed unresolved-struct field getter fallback to emit `readStruct()`/`message.StructReader` rather than generating `UnsupportedType` code paths; added regression coverage in `tests/serialization/codegen_test.zig`.
...
- 2026-02-06: Added compile-and-run coverage for generated enum-list code via `tests/serialization/codegen_generated_runtime_test.zig` (generates Zig from schema, compiles with `zig test`, and executes runtime read/write assertions), and fixed emitted enum-list getter control flow accordingly.
...
- 2026-02-06: Hardened stream/frame decode arithmetic in `src/rpc/level0/framing.zig` and message framing decode in `src/serialization/message.zig` to use checked math, returning decode errors on malformed overflowed headers instead of risking arithmetic traps.
- 2026-02-06: Added malformed/fuzz decode coverage for framing, message, and RPC protocol decode paths (`tests/rpc/level0/rpc_framing_test.zig`, `tests/serialization/message_test.zig`, `tests/rpc/level0/rpc_protocol_test.zig`) with deterministic random-input loops and malformed-header assertions.
...
- 2026-02-06: Added segment-count resource limits in message decode/validate (`Message.max_segment_count`, `ValidationOptions.segment_count_limit`) and added explicit segment-limit regression tests in `tests/serialization/message_test.zig`.
- 2026-02-06: Added traversal and nesting boundary-condition tests in `tests/serialization/message_test.zig` to lock in limit semantics (`exact-limit passes`, `below-limit fails`) for DoS hardening.
- 2026-02-06: Added looped RPC stress coverage in `src/rpc/level3/peer.zig` for embargoed `Accept` + promised-call release ordering and repeated forwarded-tail finish/return races, with explicit state-cleanup assertions for pending embargo/promise and tail-forward maps.
...
- 2026-02-06: Added additional high-load RPC cleanup coverage in `src/rpc/level3/peer.zig` for repeated third-party await/answer races and for deinit cleanup of unresolved embargoed-accept + promised-call queues.
...
- 2026-02-08: Expanded `src/rpc/level2/transport_xev.zig` failure-path coverage with EOF-close signaling, explicit close-error propagation, and write-completion behavior when callback context is missing.
- 2026-02-08: Added generated runtime schema-evolution coverage via `tests/test_schemas/schema_evolution_runtime.capnp`, proving old-writer/new-reader defaulting and new-writer/old-reader compatibility.
- 2026-02-08: Reduced duplicated return-send plumbing in `src/rpc/level3/peer/return/peer_return_dispatch.zig` by centralizing clear/send/free behavior and adding dedicated tests for exception and `takeFromOtherQuestion` frame emission.
- 2026-02-08: Optimized `MessageBuilder` write paths in `src/serialization/message.zig` by pre-reserving root storage, using contiguous text writes, and pre-sizing `toBytes()` output/header emission to reduce allocation churn without changing wire semantics.
- 2026-02-08: Added allocation-aware benchmark reporting (`alloc_calls`, `alloc_bytes`, `allocs_per_iter`, `alloc_bytes_per_iter`) in `bench/ping_pong.zig` and `bench/packed_unpacked.zig`, with new allocation-count regression cases in `bench/baselines.json`.
- 2026-02-08: Continued RPC duplication cleanup by centralizing promised-answer op-slice copying (now in `src/rpc/common/promised_answer_copy.zig`) and reusing it from `src/rpc/level0/cap_table.zig`; added alias-safety coverage in `tests/rpc/level0/rpc_cap_table_encode_test.zig`.
