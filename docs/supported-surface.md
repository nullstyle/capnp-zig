# Supported Surface (v0.18.0)

This is the authoritative statement of what `capnpc-zig` promises a consumer at
v0.17.0 — which modules are stable, what the RPC implementation conforms to, the
...
version (`zig fetch --save …#v0.18.0`) and read the CHANGELOG before bumping.
...
- **Brand fidelity is additive, not whole-program generic specialization.**
  ...
  a scalar generic binding is invalid. Generic
