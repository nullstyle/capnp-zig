...
| RPC Peer — L4 Join raw origination/readiness | `src/rpc/peer/mod.zig`, `src/rpc/peer/provide/*` | `sendJoinExperimental` can originate raw Join parts, and receive-side Join state accumulation, target equality, completion, Finish cleanup, send-failure fallback, and OOM rollback are regression covered. A test-local Zig↔Zig coordinator harness now also covers selecting/invoking the joined cap, retained-import release, mismatch without retained caps, partial-Join cancel cleanup, and callback failure after retention. The C++ L3 lane includes JoinKeyPart/JoinResult shape probes plus a source-backed blocker probe confirming no callable generic C++ L4 Join hook is available, so there is no Stable/high-level `sendJoin`, no direct-connection JoinResult flow, and no cross-implementation L4 runtime claim. See `rpc-l4-join-readiness.md`. Not frozen. |
