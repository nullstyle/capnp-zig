...
pub fn findNodeById(nodes: []schema.Node, id: schema.Id) ?*const schema.Node {
    for (nodes) |*node| {
...
pub fn findStructBySuffix(nodes: []schema.Node, suffix: []const u8) ?*const schema.Node {
    for (nodes) |*node| {
...
pub fn compareStruct(ctx: *const Context, node: *const schema.Node, reader: message.StructReader, expected: json.Value) anyerror!void {
...
fn compareSlot(ctx: *const Context, reader: message.StructReader, slot: schema.FieldSlot, expected: json.Value) anyerror!void {
    ...
    switch (slot.type) {
        ...
        .bool => {
            const bit_offset: u3 = @intCast(slot.offset % 8);
            const default_bool = if (slot.default_value) |dv| switch (dv) {
                .bool => dv.bool,
                else => false,
            } else false;
            const actual = reader.readBool(byte_offset, bit_offset) != default_bool;
            const expected_bool = try expectBool(expected);
            try std.testing.expectEqual(expected_bool, actual);
        },
        .int8 => {
            const raw = reader.readU8(byte_offset);
            const default_bits: u8 = if (slot.default_value) |dv| switch (dv) {
                .int8 => @as(u8, @bitCast(dv.int8)),
                else => 0,
            } else 0;
            const actual = @as(i8, @bitCast(raw ^ default_bits));
        ...
        .int16 => {
            ...
            const default_bits: u16 = if (slot.default_value) |dv| switch (dv) {
                .int16 => @as(u16, @bitCast(dv.int16)),
                else => 0,
            } else 0;
            const actual = @as(i16, @bitCast(raw ^ default_bits));
            const expected_i = try expectI64(expected);
            try std.testing.expectEqual(@as(i16, @intCast(expected_i)), actual);
        },
        .int32 => {
            const raw = reader.readU32(byte_offset);
            const default_bits: u32 = if (slot.default_value) |dv| switch (dv) {
                .int32 => @as(u32, @bitCast(dv.int32)),
                else => 0,
            } else 0;
            const actual = @as(i32, @bitCast(raw ^ default_bits));
            const expected_i = try expectI64(expected);
            try std.testing.expectEqual(@as(i32, @intCast(expected_i)), actual);
        },
        .int64 => {
            const raw = reader.readU64(byte_offset);
            const default_bits: u64 = if (slot.default_value) |dv| switch (dv) {
                .int64 => @as(u64, @bitCast(dv.int64)),
                else => 0,
            } else 0;
            const actual = @as(i64, @bitCast(raw ^ default_bits));
            const expected_i = try expectI64(expected);
        ...
        .float32 => {
            const raw = reader.readU32(byte_offset);
            const default_bits: u32 = if (slot.default_value) |dv| switch (dv) {
                .float32 => @as(u32, @bitCast(dv.float32)),
                else => 0,
            } else 0;
            const actual = @as(f32, @bitCast(raw ^ default_bits));
        ...
        .float64 => {
            const raw = reader.readU64(byte_offset);
            const default_bits: u64 = if (slot.default_value) |dv| switch (dv) {
                .float64 => @as(u64, @bitCast(dv.float64)),
                else => 0,
            } else 0;
            const actual = @as(f64, @bitCast(raw ^ default_bits));
            const expected_f = try expectF64(expected);
...
fn compareList(ctx: *const Context, reader: message.StructReader, pointer_index: u32, element_type: schema.Type, expected: json.Value) anyerror!void {
    ...
    switch (element_type) {
        ...
        .list => return error.UnsupportedType,
        .interface => return error.UnsupportedType,
        .any_pointer => return error.UnsupportedType,
    }
}
