...
fn run(argv: []const []const u8) ![]u8 {
    ...
    if (result.term != .exited or result.term.exited != 0) {
        ...
        return error.StreamInteropCommandFailed;
    }
...
fn runProfile(profile: capnp.codegen.Generator.ApiProfile) !void {
    ...
    const compiled = try cli.run(allocator, io, &.{ "compile", "-o-", "--src-prefix=tests/test_schemas", "tests/test_schemas/streaming.capnp" }, .{ .missing = .required });
    ...
    try writeFile(tmp.dir, "endpoint.zig", @embedFile("support/rpc_stream_endpoint.zig"));
    try writeFile(tmp.dir, "driver.cpp", @embedFile("support/rpc_stream_cpp.cpp"));
    ...
    const cpp_schema = try std.fs.path.join(allocator, &.{ directory, "streaming.capnp.c++" });
    ...
    try std.testing.expect(std.mem.indexOf(u8, output, "C++ <-> Zig deferred streaming") != null);
}
test "C++ and generated Zig stream asynchronously with bounded pressure and ordered barriers" {
